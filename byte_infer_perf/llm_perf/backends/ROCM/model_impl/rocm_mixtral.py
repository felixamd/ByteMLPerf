    ...
    def infusion_to_model(self):
        ...
        for i in range(self.model_config.num_hidden_layers):
            ...
            for j in range(self.model_config.num_local_experts):
                ...
                weights=(self.to_parameter(self.state_dict[f"model.layers.{i}.block_sparse_moe.experts.{j}.w1.weight"]),self.to_parameter(self.state_dict[f"model.layers.{i}.block_sparse_moe.experts.{j}.w3.weight"]))
                weights=torch.cat(weights, dim=0)
                self.model.model.layers[i].block_sparse_moe.w13_weight[j][:] = weights
                self.model.model.layers[i].block_sparse_moe.w2_weight[j][:] = self.to_parameter(self.state_dict[f"model.layers.{i}.block_sparse_moe.experts.{j}.w2.weight"])
